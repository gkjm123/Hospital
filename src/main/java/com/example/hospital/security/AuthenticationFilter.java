    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (token != null && securityManager.parseToken(token) != null) {
            if (token.startsWith("doctor ")) {
                SecurityContextHolder.getContext().setAuthentication(doctorDetailService.getAuthentication(token));
            }
            else if (token.startsWith("patient ")){
                SecurityContextHolder.getContext().setAuthentication(patientDetailService.getAuthentication(token));
            }
        }
